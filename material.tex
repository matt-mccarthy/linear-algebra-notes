...
\end{thm}
\begin{proof}
	Proceed via induction on $m$.
	Let $H(k)$ represent that the hypothesis that the theorem is true for $m=k$.
	Consider $H(0)$.
	In this case, $S=\emptyset$ and taking $B_1=B$ satisfies the theorem.
	Suppose $H(k)$ is true for some $k<n$, we now wish to show that $H(k+1)$ is true.
	Let $S:=\set{y_1,y_2,\ldots,s_{k+1}}$ be linearly independent.
	Since $S$ is linearly independent, $S':=\set{y_1,\ldots,y_k}$ is linearly independent.
	Since $H(k)$ is true, there exists a $B':=\set{x_1,\ldots,x_{n-k}}\subseteq B$ such that $S'\cup B'$ spans $V$.
	Therefore, there exists $\set{a_j}_{j=1}^k,\set{b_j}_{j=1}^{n-k}\subseteq\FF$ such that
	\begin{equation}\label{rep-eqn-1}
		y_{k+1}=\paren{\sum_{j=1}^k a_jy_j}+\paren{\sum_{j=1}^{n-k}b_jx_j}.
	\end{equation}
	Furthermore, at least one $b_j$ must be nonzero, because otherwise,
	\[
		y_{k+1}=\sum_{j=1}^k a_jy_j
	\]
	a clear violation of our assumed linear independence.
	Without loss of generality, assume $b_1$ is nonzero.
	Take $B_1=B'\setminus\set{x_1}$.
	Solving \autoref{rep-eqn-1} for $x_1$ yields,
	\[
		x_1=\paren{\sum_{j=1}^k b_1^{-1}a_jy_j}+\paren{\sum_{j=2}^{n-k}b_1^{-1}b_jx_j}-b_1^{-1}y_{k+1}
	\]
	which is clearly in the span of $S\cup B_1$.
	Furthermore, $y_1,\ldots,y_m,y_{m+1},x_2,\ldots,x_{n-k}$ is also in that span.
	By minimality (Theorem\autoref{lincomb-min-ss}), we have
	\[
		\spann (S'\cup B') \subseteq \spann (S\cup B_1) \subseteq V.
	\]
	However, $\spann (S'\cup B')=V$ by $H(K)$ and thus,
	\[
		V\subseteq \spann (S\cup B_1) \subseteq V.
	\]
	Ergo, $\spann (S\cup B_1) = V$.
	Since $S\cup B_1$ is linearly independent as well, $S\cup B_1$ is a basis.
\end{proof}
...
\end{corollary}
\begin{proof}
	Let $S\subseteq V$ be linearly independent with $\card S=n$.
	Then by Theorem\autoref{replacement-thm} there exists a $B_1\subseteq B$ with $\card B_1=n-n=0$ such that $B_1\cup S$ is a basis for $V$.
	Since $\card B_1=0$, $B_1=\emptyset$ and $S$ is a basis for $V$.
\end{proof}
...
\end{corollary}
\begin{proof}
	Suppose $S\subseteq V$ is linearly independent with $\card S > n$.
	Therefore any $R\subset S$ with $\card R=n$ is a basis of $V$.
	Moreover, since $\card R < \card S$ and $R\subset S$, there exists an $x\in S\setminus R$.
	Since $R$ is a basis, and $x\in V$, $x\in \spann R$, which in turn tells us that $S$ is linearly dependent.
	Therefore, any $S\subseteq V$ with $\card S > n$ is linearly dependent.
\end{proof}
...
\begin{proof}
	...
	Since $B$ is a basis, it is linearly independent and thus it has cardinality less than or equal to $n$.
	Define $m=\card S$.
	We know that $m \leq n$ since $S$ is linearly independent and $B$ is a basis.
	Moreover, $n leq m$ since $B$ is linearly independent and $S$ is a basis.
	Thus, $m=n$ and any basis of $V$ will have cardinality $n$.
\end{proof}

\begin{definition}[Dimension of a Vector Space]
	A vector space $V$ is called \textit{finite-dimensional} if and only if $V$ has a finite basis.
	Furthermore $B$ is a basis of $V$ with $\card B=n$ then we say $V$ has \textit{dimension} $n$, denoted $\dim V = n$.
	If $V$ is not finite-dimensional, then we say $V$ is \textit{infinite-dimensional}.
\end{definition}
...
\end{corollary}
\begin{proof}
	Since $\spann S=V$, there exists an $S'\subseteq S$ such that $S'$ is a basis for $V$.
	Since $S'$ is a basis and $\dim V=n$, $\card S'=n$.
	Thus,
	\[
		n = \card S' \leq \card S \leq n.
	\]
	Yielding, $\card S=n$ and $S' = S$.
	Therefore, $S$ is a basis of $V$.
\end{proof}

\begin{corollary}
	Let $B$ be a basis for a finite-dimensional vector space $V$ and let $S\subseteq V$ be linearly independent.
	Then there exists a $B'\subseteq B$ such that $B'\cup S$ is a basis of $V$.
\end{corollary}
\begin{proof}
	Define $\dim V=n$ and $\card S=m\leq n$.
	Then by replacement theorem, there exists a $B'\subseteq B$ with $\card B' = n-m$ such that $B'\cup S$ is a basis of $V$.
\end{proof}
...
\end{thm}
\begin{proof}
	Let $B$ be a basis for $W$.
	Then $B$ is linearly independent.
	Since $B$ is linearly independent and $B\subseteq W\subseteq V$, $\dim W=\card B\leq \dim V$.

	Suppose $\dim W=\dim V$.
	Let $B$ be a basis for $W$.
	Since $B$ is linearly independent and $\card B=\dim W=\dim V$, $B$ is a basis for $V$.
	Thus, $W=\spann B = V$.
\end{proof}
...
\end{corollary}
\begin{proof}
	Let $S$ be a basis of $W$ and $B$ be a basis of $V$.
	Then $S$ is linearly independent and by replacement theorem there exists a $B'\subseteq B$ such that $S\cup B'$ is a basis of $V$.
	Obviously, $S\subseteq S\cup B'$ and thus $S$ is a subset of a basis of $V$.
\end{proof}
